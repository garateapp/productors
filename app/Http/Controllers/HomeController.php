...
public function uploadAndReadExcelGreenvic(Request $request)
    ...
    public function viewpdf(Recepcion $recepcion) {
        ...
        if ($recepcion->calidad->detalles->where('tipo_item','COLOR DE FONDO')->count()) {
            $distribucion_color_fondo='https://v1.nocodeapi.com/greenex/screen/CbrYLdYsupiNNAot/screenshot?url=https://appgreenex.cl/color/fondo/'.$recepcion->id.'.html&delay=1&viewport=600x420';
        }else{
