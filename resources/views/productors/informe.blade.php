...
<body>
	...
	<div class="container">
		...
		<h1 style="background-color: rgb(0,0,0,0.5);" >Informe de Recepcion Guia N° {{$recepcion->numero_documento_recepcion}} | {{$recepcion->n_especie}} | <br> {{$recepcion->n_variedad}} | Cuartel GE001 | CSG <br> {{$recepcion->Codigo_Sag_emisor}} </h1>
	</div>
		<table style="width:100%; background-color:#ececec;">
			...
			if ($recepcion->n_especie=='Orange') {
				...
				if ($recepcion->calidad->detalles->where('tipo_item','DISTRIBUCIÓN DE CALIBRES')->where('detalle_item','>80mm')->first()) {
					$b=$recepcion->calidad->detalles->where('tipo_item','DISTRIBUCIÓN DE CALIBRES')->where('detalle_item','>80mm')->first()->porcentaje_muestra;
				}
			}else{	
